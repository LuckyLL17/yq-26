...
export default function TowerUpgradePanel() {
  ...
  return (
    <div className="bg-game-panel/95 backdrop-blur-sm rounded-xl p-4 border border-yellow-500/40 shadow-lg shadow-yellow-500/10 relative">
      <button
        onClick={handleClose}
        className="absolute top-2 right-2 text-gray-400 hover:text-white transition-colors text-lg w-6 h-6 flex items-center justify-center rounded hover:bg-white/10"
      >
        ×
      </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-0.5">
              {Array.from({ length: config.maxLevel }).map((_, i) => (
                <span
                  key={i}
                  className={`text-sm ${i < tower.level ? 'text-yellow-400' : 'text-gray-600'}`}
                >
                  ★
                </span>
              ))}
      ...
      <div className="space-y-2">
        {!isMaxLevel ? (
          <button
            ...
            className={`
              ...
              ${canUpgrade
                ? 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white hover:from-yellow-400 hover:to-orange-400 hover:scale-105 shadow-lg shadow-yellow-500/30 active:scale-100'
                : 'bg-gray-700 text-gray-400 cursor-not-allowed'
              }
            `}
          >
            <span>⬆️</span>
            <span>升级到 {tower.level + 1} 级</span>
            <span className="ml-1">💰 {upgradeCost}</span>
          </button>
        ) : (
          <div className="w-full py-3 px-4 rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 text-white text-center font-bold text-sm shadow-lg shadow-purple-500/30">
            ✨ 已达最高等级 ✨
          </div>
        )}
        ...
        >
          <span>💸</span>
          <span>出售</span>
          <span className="ml-1">+💰 {sellValue}</span>
        </button>
      </div>
